...
const createBasket=async function(req,res){
    try{
        ...
        if (!isValidString(item)) return res.status(400).send({ status: false, message: "Please provide data" });
        ...
        const basketList=await basketModel.findOneAndUpdate({pantryId:req.params.pantryId,basketName:req.params.basketName},{item:item},{new:true});
